...
from meld_data import MELDDataset
...
class MultimodalTrainer:
    def __init__(self, model, train_loader, val_loader):
        ...
        timestamp = datetime.now().strftime('%b%d_%H-%M-%S')  # e.g., Mar17_12-32-02
        base_dir = '/opt/ml/output/tensorboard' if 'SM_MODEL_DIR' in os.environ else 'runs'
        log_dir = f"{base_dir}/run_{timestamp}"
        self.writer = SummaryWriter(log_dir=log_dir)
        self.global_step = 0  # Fixed typo: "global_step" instead of "gobal_step"
    ...
    def validate(self, val_loader, phase="val"):
        ...
        avg_loss = {k: v / len(val_loader) for k, v in val_loss.items()}
...
if __name__ == "__main__":
    dataset = MELDDataset(
        ...
        r"D:\sentiment aanalysis\dataset\train\train_splits"
    )
